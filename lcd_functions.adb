...
package body LCD_Functions is
   ...
   begin
      ...
      while Update_Is_Required loop null; end loop;

      for J in 1 .. L loop
         LCD_Driver.LCD_Text_Buffer (J) := Element (Text, J);
      end loop;

      LCD_Text_Buffer (L + 1) := ASCII.NUL;

      if L > 5 then
         Scroll_Mode := Wave;   -- Scroll if text is longer than display size
         Scroll_Offset := 0;
         Start_Scroll_Delay := 1; -- Start-up delay before scrolling the text
      else
         Scroll_Mode   := None;
         Scroll_Offset := 0;
      end if;

      LCD_Driver.Update_Is_Required := True;
   end Put;
   ...
   begin
      if Base /= 16 then
         ...
         LCD_Driver.LCD_Text_Buffer (L) := Character'Val (N + 48);
         L := L - 1;

         if H > 0 then
            LCD_Driver.LCD_Text_Buffer (L) := Character'Val (T + 48);
            L := L - 1;
            LCD_Driver.LCD_Text_Buffer (L) := Character'Val (H + 48);
         else
            if T > 0 then
               LCD_Driver.LCD_Text_Buffer (L) := Character'Val (T + 48);
            else
               LCD_Driver.LCD_Text_Buffer (L) := ' ';
            end if;
            L := L - 1;
            LCD_Driver.LCD_Text_Buffer (L) := ' ';
         end if;
         ...
         null;
         --  hexadecimal
         --U8_Hex_Img (Num, LCD_Driver.LCD_Text_Buffer (L - 1 .. L));
      end if;
      LCD_Driver.Update_Is_Required := True;
   end Put;
   ...
   begin
      ...
      LCD_Driver.LCD_Text_Buffer (L) := Character'Val (H+48);
      L := L - 1;
      LCD_Driver.LCD_Text_Buffer (L) := Character'Val (TH+48);
      L := L - 1;
      LCD_Driver.LCD_Text_Buffer (L) := Character'Val (TT+48);
      LCD_Driver.Update_Is_Required := True;
   end Put;
